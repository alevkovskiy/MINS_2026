...
func (s *BulkDiscountStrategy) GetName() string {
	return fmt.Sprintf("Оптовая скидка %.0f%% (от %d шт.)", s.DiscountPercent, s.MinQuantity)
}
...
func (s *CategoryDiscountStrategy) GetName() string {
	return fmt.Sprintf("Категорийная скидка %.0f%% (%s)", s.DiscountPercent, s.Category)
}
...
func (s *SeasonDiscountStrategy) GetName() string {
	return fmt.Sprintf("Сезонная скидка %.0f%%", s.DiscountPercent)
}
