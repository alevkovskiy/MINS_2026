...
type ConsoleHandler struct {
	...
	reader     *bufio.Reader
	factoryReg *domain.ProductFactoryRegistry
}

func NewConsoleHandler(svc *service.WarehouseService, repo repository.Repository) *ConsoleHandler {
	return &ConsoleHandler{
		service:    svc,
		repo:       repo,
		reader:     bufio.NewReader(os.Stdin),
		factoryReg: domain.NewProductFactoryRegistry(),
	}
}
...
func (h *ConsoleHandler) Run() error {
	...
	fmt.Println("\n" + strings.Repeat("=", 60))
	fmt.Println("     СИСТЕМА УПРАВЛЕНИЯ СКЛАДОМ (Core Service)")
...
func (h *ConsoleHandler) printMenu() {
	...
	// Показываем статус Service B
	if h.service.IsServiceBAvailable() {
		fmt.Println("✅ Service B (Reference Service) доступен")
	} else {
		fmt.Println("⚠️  Service B (Reference Service) НЕДОСТУПЕН - проверка совместимости отключена")
	}
}

func (h *ConsoleHandler) generateTraceID() string {
...
func (h *ConsoleHandler) addProductInteractive() {
	...
	fmt.Printf("\n[TraceID: %s] ДОБАВЛЕНИЕ НОВОГО ТОВАРА\n", traceID)
	...
	categories := h.getCategoriesWithFallback(traceID)
	...
	fmt.Print("\nВыберите категорию (1-" + strconv.Itoa(len(categories)) + "): ")
	...
	if err := h.service.AddProduct(product, traceID); err != nil {
		switch e := err.(type) {
		...
		if !h.service.IsServiceBAvailable() {
			fmt.Println("⚠️  ВНИМАНИЕ: Service B был недоступен, проверка совместимости не выполнялась")
		}
	}
}

func (h *ConsoleHandler) getCategoriesWithFallback(traceID string) []string {
	categories := h.service.GetCategoriesFromServiceB(traceID)
	if len(categories) == 0 {
		return h.factoryReg.GetCategories()
	}
	return categories
}
...
func (h *ConsoleHandler) checkCompatibility() {
	...
	if !h.service.IsServiceBAvailable() {
		fmt.Println("⚠️  Service B (Reference Service) недоступен!")
		fmt.Println("   Проверка совместимости невозможна.")
		fmt.Println("   Пожалуйста, убедитесь, что Service B запущен на порту :50051")
		return
	}

	fmt.Println("Выберите категорию для проверки:")
	fmt.Println("1. Food")
	fmt.Println("2. Electronics")
	fmt.Println("3. Chemicals")
	...
	existing := h.service.GetAllProducts()
	existingCategories := make([]string, 0, len(existing))
	catMap := make(map[string]bool)
	for _, p := range existing {
		if !catMap[p.GetCategory()] {
			catMap[p.GetCategory()] = true
			existingCategories = append(existingCategories, p.GetCategory())
		}
	}

	compatible, msg, _ := h.service.CheckCompatibilityWithServiceB(traceID, category, existingCategories)

	fmt.Println("\n" + strings.Repeat("-", 50))
	if compatible {
		fmt.Printf("✅ Товары категории '%s' совместимы со всеми товарами на складе\n", category)
	} else {
		fmt.Printf("❌ Товары категории '%s' НЕ совместимы!\n", category)
...
func (h *ConsoleHandler) addTestData() {
	...
	foodFactory := domain.NewFoodFactory()
	electronicsFactory := domain.NewElectronicsFactory()
	chemicalsFactory := domain.NewChemicalsFactory()
	...
	h.service.AddProduct(food, "test-init-1")
	h.service.AddProduct(electronics, "test-init-2")
	h.service.AddProduct(chemicals, "test-init-3")
}
